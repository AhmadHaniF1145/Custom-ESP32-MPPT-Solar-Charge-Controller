...
void Read_Sensors(){
  ...
  int adcValue = analogRead(TempSensor); // Membaca nilai ADC
  float Vout = (adcValue / 4095.0) * 3.3; // Menghitung tegangan dari nilai ADC (ESP32 menggunakan 12-bit ADC)
  float Rntc = Rref * (3.3 / Vout - 1); // Menghitung resistansi NTC

  // Menghitung suhu menggunakan rumus Steinhart-Hart
  float TSlog = log(Rntc);
