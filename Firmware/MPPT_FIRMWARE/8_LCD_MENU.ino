...
void LCD_Menu(){
  ...
  //MAIN MENU
    ...
    //BUTTON KEYPRESS
    if(digitalRead(buttonRight)==0) {buttonRightCommand=0;lcdBacklight_Wake();}
    if(digitalRead(buttonLeft)==0)  {buttonLeftCommand=0;lcdBacklight_Wake();}
    if(digitalRead(buttonBack)==0)  {buttonBackCommand=0;lcdBacklight_Wake();}
    if(digitalRead(buttonSelect)==0){buttonSelectCommand=0;lcdBacklight_Wake();}
    ...
    if(currentLCDMillis-prevLCDMillis>=millisLCDInterval&&enableLCD==1){   //Run routine every millisLCDInterval (ms)
      ...
      if(buttonRightCommand==0)      {buttonRightCommand=1;menuPage++;lcd.clear();}
      else if(buttonLeftCommand==0)  {buttonLeftCommand=1;menuPage--;lcd.clear();}
      else if(buttonBackCommand==0)  {buttonBackCommand=1;menuPage=0;lcd.clear();}
      else if(buttonSelectCommand==0&&menuPage==4){buttonSelectCommand=1;settingMode=1;lcd.clear();}
