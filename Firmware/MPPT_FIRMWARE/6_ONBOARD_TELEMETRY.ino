void Onboard_Telemetry(){    
  ...
  if(currentSerialMillis-prevSerialMillis>=millisSerialInterval){   //Run routine every millisRoutineInterval (ms)
    ...
    if (digitalRead(buttonLeft) == 0) { // Tombol kiri ditekan
        Serial.println("Tombol Kiri Ditekan");
        // Serial.print(buttonLeftCommand);
        // Serial.print(buttonLeft );
        digitalWrite(LED, HIGH); // Nyalakan LED
        delay(300); // Delay untuk debouncing
    } else if (digitalRead(buttonRight) == 0) { // Tombol kanan ditekan
        Serial.println("Tombol Kanan Ditekan");
        digitalWrite(LED, HIGH); // Nyalakan LED
        delay(300); // Delay untuk debouncing
    } else if (digitalRead(buttonBack) == 0) { // Tombol kembali ditekan
        Serial.println("Tombol Kembali Ditekan");
        digitalWrite(LED, HIGH); // Nyalakan LED
        delay(300); // Delay untuk debouncing
    } else if (digitalRead(buttonSelect) == 0) { // Tombol pilih ditekan
        Serial.println("Tombol Pilih Ditekan");
        digitalWrite(LED, HIGH); // Nyalakan LED
        delay(300); // Delay untuk debouncing
    } else {
        digitalWrite(LED, 0); // Matikan LED jika tidak ada tombol yang ditekan
    }
...
//  else if(chargingPause==1){Serial.println("CHARGING PAUSED");}   // Charging paused message
      ...
      Serial.print (" Vin adc:"); Serial.print(ads.readADC_SingleEnded(3));   
      Serial.print (" Vout adc:"); Serial.print(ads.readADC_SingleEnded(1));
      // Serial.print (" Current adc:"); Serial.print(ads.readADC_SingleEnded(2));                        
